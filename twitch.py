...
async def dispatch_alert(alert: dict | None) -> None:
    if not alert:
        return
...
async def handle_notification(stype: str, ev: dict) -> None:
    ...
    if stype == "channel.follow":
        await dispatch_alert(build_alert("follow", user=user))

    elif stype == "channel.subscribe":
        await dispatch_alert(build_alert("subscribe", user=user, tier=str(int(ev.get("tier", "1000")) // 1000)))
        ...
        await dispatch_alert(build_alert("hypetrain", amount=str(ev.get("level", 1))))

    elif stype == "stream.online":
        await dispatch_alert(build_alert("online"))

    elif stype == "stream.offline":
        await dispatch_alert(build_alert("offline"))
            ...
            async with websockets.connect(url, max_size=2 ** 22) as ws:
                ...
                async for raw in ws:
                    ...
                    if mtype == "session_welcome":
                        ...
                        await handle_notification(stype, payload.get("event", {}) or {})
